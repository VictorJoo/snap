...
var ExtensionDialogMorph; /*added sjl*/
...
ExtensionDialogMorph.prototype = new DialogBoxMorph();
ExtensionDialogMorph.prototype.constructor = ExtensionDialogMorph;
ExtensionDialogMorph.uber = DialogBoxMorph.prototype;

// BlockDialogMorph instance creation:

function ExtensionDialogMorph(target, action, environment) {
    this.init(target, action, environment);
}

ExtensionDialogMorph.prototype.init = function (target, action, environment) {
    // additional properties:
    this.blockType = 'command';
    this.category = 'other';
    this.isGlobal = true;
    ...
    // initialize inherited properties:
    ExtensionDialogMorph.uber.init.call(
        this,
        target,
        action,
        environment
    ...
    this.categories.borderColor = this.categories.color.lighter(40);
    this.createCategoryButtons();
    this.fixCategoriesLayout();         /*sjl*/
    this.add(this.categories);

    /*this.createTypeButtons();
    this.createScopeButtons();*/        /*sjl*/
    this.fixLayout();
};

ExtensionDialogMorph.prototype.openForChange = function (
    title,
    category,
    type,
    world,
...
ExtensionDialogMorph.prototype.createCategoryButtons = function () {
    var myself = this,
        oldFlag = Morph.prototype.trackChanges;
...
ExtensionDialogMorph.prototype.addCategoryButton = function (category) {
    var labelWidth = 75,    //각 category 길이
        myself = this,
        colors = [
            SpriteMorph.prototype.paletteColor,
            SpriteMorph.prototype.paletteColor.darker(50),
...
ExtensionDialogMorph.prototype.fixCategoriesLayout = function () {
    var buttonWidth = this.categories.children[0].width(), // all the same
        buttonHeight = this.categories.children[0].height(), // all the same
        xPadding = 15,
        yPadding = 2,
...
ExtensionDialogMorph.prototype.createTypeButtons = function () {
    var block,
        myself = this,
        clr = SpriteMorph.prototype.blockColor[this.category];
...
ExtensionDialogMorph.prototype.addBlockTypeButton = function (
    action,
    element,
    query
) {
...
ExtensionDialogMorph.prototype.addTypeButton = function (action, label, query) {
    var button = new ToggleMorph(
        'radiobutton',
        this,
        action,
...
ExtensionDialogMorph.prototype.setType = function (blockType) {
    this.blockType = blockType || this.blockType;
    this.types.children.forEach(function (c) {
        c.refresh();
    });
...
ExtensionDialogMorph.prototype.createScopeButtons = function () {
    var myself = this;
...
ExtensionDialogMorph.prototype.addScopeButton = function (action, label, query) {
    var button = new ToggleMorph(
        'radiobutton',
        this,
        action,
...
ExtensionDialogMorph.prototype.setScope = function (varType) {
    this.isGlobal = (varType === 'gobal');
    this.scopes.children.forEach(function (c) {
        c.refresh();
    });
...
ExtensionDialogMorph.prototype.fixLayout = function () {
    var th = fontHeight(this.titleFontSize) + this.titlePadding * 2;
