...
IDE_Morph.prototype.createSpriteBar = function () {
    ...
    tab = new TabMorph(     //added sjl  contents탭 추가
        tabColors,
        null, // target
        function () {tabBar.tabTo('contents'); },
        localize('Contents'), // label
        function () {  // query
            return myself.currentTab === 'contents';
        }
    );
    tab.padding = 3;
    tab.corner = tabCorner;
...
IDE_Morph.prototype.createSpriteEditor = function () {
    ...
    if (this.currentTab === 'scripts') {
    ...
    }else if (this.currentTab === 'contents'){ // added sjl

        this.spriteEditor = new ContentsboxMorph(
        ...
        //added sjl open iframe
        var div = document.createElement('DIV');
        var ifrm = document.createElement('IFRAME');

        div.setAttribute("id","frameDiv");
        ifrm.setAttribute("src", "http://106.243.134.230:2369/");
        ifrm.setAttribute("id", "victor");

        ifrm.style.width = 100+"%";
        ifrm.style.height = 100+"%";

        div.style.width = document.body.clientWidth - this.stage.width() - 220 + "px";
...
//added sjl remove iframe
function rmIframe(){
    if( document.getElementById('frameDiv')){
        var olddiv = document.getElementById("frameDiv");
...
IDE_Morph.prototype.fixLayout = function (situation) {
    ...
    if (situation !== 'refreshPalette') {
        ...
        if (this.isAppMode) {
            ...
            //added sjl 15.11.12 isAppMode가 비활성시 contents 탭일 경우 crate iframe
        ...
        if (!contains(['selectSprite', 'tabEditor'], situation)) {
            ...
            //added sjl corral width 변화 에 따른 contents width 구하기
...
IDE_Morph.prototype.snapMenu = function () {
    ...
    if(this.currentTab === 'contents')      //added sjl 15.11.12  컨텐츠 화면일시 scripts로 이동
...
IDE_Morph.prototype.settingsMenu = function () {
    ...
    //added sjl setting button popup 위치조정 15.11.12
   /* if( document.getElementById('frameDiv')){
        var pos = new Point(33, 26);
    }else*/
        var  pos = this.controlBar.settingsButton.bottomLeft();
...
IDE_Morph.prototype.projectMenu = function () {
    ...
    //added sjl project menu button 조정 15.11.12
    /*if( document.getElementById('frameDiv')){
        var pos = new Point(93, 26);
    }else*/
...
// Contents service ///////////////////////////////////////////// added sjl
ContentsboxMorph.prototype = new ScrollFrameMorph();
ContentsboxMorph.prototype.constructor = ContentsboxMorph;
ContentsboxMorph.uber = ScrollFrameMorph.prototype;

function ContentsboxMorph(sliderColor) {
    this.init(sliderColor);
}
ContentsboxMorph.prototype.init = function (sliderColor) {
    // additional properties
    this.sprite = new SpriteMorph();
    this.costumesVersion = null;
    this.spriteVersion = null;

    // initialize inherited properties
    ContentsboxMorph.uber.init
        .call(this, null, null, sliderColor);
...
// ContentsboxMorph updating
ContentsboxMorph.prototype.updateList = function () {
    var myself = this,
        x = this.left() + 5,
        y = this.top() + 5,
        padding = 4,
